...
luca is a server that owns one SQLite file and is the only process that writes to it. It keeps the books of one société: écritures in journaux, in double entry, one exercice per file. Every client — a script, a human in a chat, a language model — talks to the server over HTTP or MCP, on one port, with five names: `/add`, `/query`, `/compte`, `/journal`, `/exercice` (`luca_add`, `luca_query`, `luca_add_compte`, `luca_add_journal`, `luca_open_exercice`). The only command is `luca serve`.
