...
1. **One file, one société, one exercice.** Journaux and comptes are société-wide. The exercice table holds at most one row, enforced by the schema.
...
3. **Accepted is immutable.** Triggers refuse every `UPDATE` and `DELETE` on `ecriture` and `ligne`. The only correction is an inverse écriture linked to the one it cancels ([annule.md](annule.md)).
4. **Two identities.** `ecriture.id` is technical: stable, never reused, meaningless. `num` is accounting: `EcritureNum`, continuous per journal, assigned inside the accepting transaction.
